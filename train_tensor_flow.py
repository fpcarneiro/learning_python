...
test = get_numeric_columns(['teste1', 'test2'])  
    
for c in X_train.iteritems():
    print(c)
    #print(type(c))
    
    
t = X_train.columns.to_series().groupby(X_train.dtypes).groups
                             
teste = get_base_numeric_columns(X_train, types_included = ['float32', 'float64'])

columns = list(X_train.select_dtypes(include = ['float32', 'float64']).columns)


list(X_train.select_dtypes(include = ['float32', 'float64']).columns)